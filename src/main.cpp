...
#if defined(NDEBUG)
# error "Emerald cannot be compiled without assertions."
#endif
...
{
    // Emerald: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 60 * 60; // Emerald: 1 hour
static const int64 nTargetSpacing = 20;       // Emerald: 20 seconds
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        printf("hash to string  %s\n", hash.ToString().c_str());
        printf("hash genesis bk %s\n", hashGenesisBlock.ToString().c_str());
        printf("hash merkle rt  %s\n", block.hashMerkleRoot.ToString().c_str());
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EmeraldMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EmeraldMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EmeraldMiner(CWallet *pwallet)
{
    printf("EmeraldMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("emerald-miner");
    ...
    try { loop {
        ...
        printf("Running EmeraldMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EmeraldMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EmeraldMiner, pwallet));
}
